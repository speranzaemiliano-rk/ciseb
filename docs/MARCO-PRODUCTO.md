...
| **RBAC efectivo / SSO Workspace** | 🟡 | Login con Google agregado (`loginConGoogle()`), restringible a un dominio de Workspace (`config.js → dominioWorkspace`, parámetro `hd` + re-chequeo del email post-login). Roles siguen aplicándose **solo en cliente** — la restricción de dominio es acceso/UX, no un gate server-side (haría falta una Blocking Function de Firebase Auth, plan Blaze). Las reglas de Firebase siguen siendo la única defensa real de los datos. |
...
| **1 — Núcleo clínico** | Login+RBAC, ficha, historia clínica, **evolución→odontograma** (FDI, 3 capas, por cara), adjuntos y portal de signed URLs | 🟡 ficha, adjuntos, **evolución→odontograma** (2 de 3 capas), **portal de signed URLs** y **login con Google/Workspace** ya cerrados; falta RBAC efectivo (server-side) |
